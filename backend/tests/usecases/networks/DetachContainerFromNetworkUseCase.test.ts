...
import { DetachContainerFromNetworkUseCase } from '../../../src/application/usecases/networks/impl/DetachContainerFromNetworkUseCase.js';
...
describe('DetachContainerFromNetworkUseCase', () => {
    ...
    it('throws when instance not attached', async () => {
        ...
        await expect(usecase.execute('net-1', 'inst-1')).rejects.toThrow(/not attached/);
    });

    it('disconnects and detaches', async () => {
        (networkRepo.getById as any).mockReturnValue(makeNetwork({ dockerId: 'd-1', instance: ['inst-1'] }));
        (instanceRepo.getById as any).mockReturnValue(makeInstance({ id: 'inst-1', containerId: 'c-1' }));

        await usecase.execute('net-1', 'inst-1');

        expect(dockerPort.disconnectContainer).toHaveBeenCalledWith('d-1', 'c-1');
        expect(networkRepo.detachContainer).toHaveBeenCalledWith('net-1', 'inst-1');
    });
});
