...
import { IDetachContainerFromNetworkUseCase } from "../IDetachContainerFromNetworkUseCase.js";

export class DetachContainerFromNetworkUseCase implements IDetachContainerFromNetworkUseCase {
    ...
    async execute(networkId: string, instanceId: string): Promise<Network> {
        ...
        // Check if the instance is actually attached to this network
        if (!network.instance.includes(instanceId)) {
            throw new Error(`Instance "${instanceId}" is not attached to network "${network.name}"`);
        }

        await this.dockerPort.disconnectContainer(network.dockerId, instance.containerId);

        return this.networkRepository.detachContainer(networkId, instanceId);
    }
}
